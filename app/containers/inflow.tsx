...
const Inflow = () => {
  ...
  const AddInFlowTransaction = async () => {
    ...
    if (isValid) {
      setLoading(true);
      try {
        const response = await addInFlowTransaction(email, fund, 'a', accNo, mot, instrumentType, beneData.account_name, beneData.bank_name, beneData.branch_name, beneData.account_title, instrumentDate, instrumentNo, grossAmount, des, netAmount, amcName);
        setAmcName('');
        setFund('');
        setAccNo('');
        setFilioNo('');
